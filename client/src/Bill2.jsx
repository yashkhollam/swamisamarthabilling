...
function Bill2() {
  ...
  const submitform = async (e) => {
    ...
    try {
      ...
      // Reset product & invoice fields only
  ...
  return (
    <>
      <div className="container-fluid" id='bill-cont'>
        <div className="row" id='bill-row'>
          <div className="col-12" id='bill-col'>
            ...
            <form id='form' onSubmit={submitform}>
              ...
              <div id='btn-cont'>
                <button type='button' id='addbtn' onClick={addProduct}>Add data</button>
                <button type='submit' id='submitbtn'>Submit Bill</button>
              </div>
            </form>
          </div>
        </div>
              ...
              <div className="card">
                ...
                <div id='data-cont'>
                  <div id='data-price-cont' className='mt-2'>
                    <span id='label' style={{ fontWeight: "bold" }}>Price:</span>
                    <span className='ms-2'>{data.price}</span>
                    <span id='label' style={{ fontWeight: "bold", marginLeft: "20px" }}>Qty:</span>
                    <span className='ms-2'>{data.quantity}</span>
                  </div>

                  <div>
                    <button id='editbtn' onClick={() => openEditModal(data, index)}>edit</button>
                    <button id='deletebtn' onClick={() => deleteitem(data.billId)}>delete</button>
                  </div>
                </div>

                <div id='total-cont'>total : <p id='total'>{data.total}</p></div>
              </div>
            </div>
          )) : <h1>no data</h1>}
        ...
        <div className="edit-modal">
          <div className="edit-modal-content">
            ...
            <input
              ...
              onChange={(e) => setEditingProduct(prev => ({ ...prev, quantity: e.target.value }))}
            />
            <input
              type="number"
              placeholder="Price"
            ...
            <div className="modal-buttons">
              <button
               ...
               style={{backgroundColor:"yellow"}}>Cancel</button>
